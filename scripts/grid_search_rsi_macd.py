...
from shared.agents.rsi_macd_agent import RSIMACDAgent
from shared.agents.rsi_macd_agent_with_stoploss import RSIMACDAgentWithStopLoss
...
def run_single_simulation(
    ...
    if stop_loss_percentage is not None:
        # 損失確定機能付き
        agent = RSIMACDAgentWithStopLoss(
            agent_id=agent_id,
...
def grid_search(
    ...
    # すべての組み合わせを生成
    valid_combinations = []
    for (rsi_p, rsi_os, rsi_ob, macd_f, macd_s, macd_sig, stop_loss, trailing) in itertools.product(
        rsi_periods,
        rsi_oversold_levels,
        ...
        # lookback_windowはMACDスロー期間とシグナル期間の合計以上が必要
        min_lookback_required = max(macd_s + macd_sig, rsi_p + 1, min_lookback)
        if min_lookback_required + 100 < len(price_data):
            valid_combinations.append((rsi_p, rsi_os, rsi_ob, macd_f, macd_s, macd_sig, stop_loss, trailing, min_lookback_required))
    ...
    for idx, (rsi_p, rsi_os, rsi_ob, macd_f, macd_s, macd_sig, stop_loss, trailing, lookback) in enumerate(valid_combinations, 1):
        ...
        agent_id = f"rsi_macd_r{rsi_p}_os{rsi_os:.0f}_ob{rsi_ob:.0f}_f{macd_f}_s{macd_s}_sig{macd_sig}_{stop_loss_str}_{trailing_str}"
    ...
    print("トップ20の結果:")
    print("-" * 140)
    header = f"{'Rank':<5} {'RSI':<4} {'RSI OS':<7} {'RSI OB':<7} {'MACD F':<7} {'MACD S':<7} {'MACD Sig':<9} {'SL':<6} {'TS':<6} {'Profit%':<12} {'Profit$':<12} {'Trades':<8}"
    print(header)
    print("-" * 140)
    ...
    for rank, result in enumerate(results[:20], 1):
        ...
        ts_str = f"{result['trailing_stop_percentage']*100:.0f}%" if result.get('trailing_stop_percentage') else "なし"
        print(f"{rank:<5} "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="RSI+MACDエージェントのグリッドサーチ")
    parser.add_argument("--csv", type=str, default="data/btc_prices_2021_2025.csv")
    parser.add_argument("--rsi-period", type=int, nargs='+', default=[14, 21, 28], help="RSI期間")
    parser.add_argument("--rsi-oversold", type=float, nargs='+', default=[20, 30, 40], help="RSIオーバーソールド閾値")
    parser.add_argument("--rsi-overbought", type=float, nargs='+', default=[60, 70, 80], help="RSIオーバーボート閾値")
    parser.add_argument("--macd-fast", type=int, nargs='+', default=[12, 8, 6], help="MACD短期EMA期間")
    parser.add_argument("--macd-slow", type=int, nargs='+', default=[26, 21, 13], help="MACD長期EMA期間")
    parser.add_argument("--macd-signal", type=int, nargs='+', default=[9, 7, 5], help="MACDシグナルライン期間")
