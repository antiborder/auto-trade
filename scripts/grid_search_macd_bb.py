...
from shared.agents.macd_bb_agent import MACDBBAgent
...
def grid_search(
    ...
    # すべての組み合わせを生成
    valid_combinations = []
    for (macd_f, macd_s, macd_sig, bb_p, bb_std) in itertools.product(
        macd_fast_periods,
        macd_slow_periods,
        macd_signal_periods,
        bb_periods,
        ...
        min_lookback_required = max(macd_s + macd_sig, bb_p, min_lookback)
        if min_lookback_required + 100 < len(price_data):
            valid_combinations.append((macd_f, macd_s, macd_sig, bb_p, bb_std, min_lookback_required))
    ...
    for idx, (macd_f, macd_s, macd_sig, bb_p, bb_std, lookback) in enumerate(valid_combinations, 1):
        agent_id = f"macd_bb_f{macd_f}_s{macd_s}_sig{macd_sig}_bbp{bb_p}_bbstd{bb_std}"
        ...
        try:
            ...
            error_msg = f"エラー: MACD={macd_f}/{macd_s}/{macd_sig}, BB={bb_p}/{bb_std}: {e}"
            print(error_msg)
            if log_path:
                write_log(error_msg)
            continue
    ...
    print("トップ20の結果:")
    print("-" * 140)
    header = f"{'Rank':<5} {'MACD F':<7} {'MACD S':<7} {'MACD Sig':<9} {'BB P':<6} {'BB Std':<7} {'Profit%':<12} {'Profit$':<12} {'Trades':<8}"
    print(header)
    print("-" * 140)
    ...
    if log_path:
        ...
        if best_result:
            ...
            write_log(f"Best parameters: MACD({best_result['macd_fast']}/{best_result['macd_slow']}/{best_result['macd_signal']}), BB({best_result['bb_period']}/{best_result['bb_num_std_dev']})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MACD+ボリンジャーバンドエージェントのグリッドサーチ")
    parser.add_argument("--csv", type=str, default="data/btc_prices_2021_2025.csv")
    parser.add_argument("--macd-fast", type=int, nargs='+', default=[8, 12, 16], help="MACD短期EMA期間")
    parser.add_argument("--macd-slow", type=int, nargs='+', default=[21, 26, 31], help="MACD長期EMA期間")
    parser.add_argument("--macd-signal", type=int, nargs='+', default=[7, 9, 11], help="MACDシグナルライン期間")
    parser.add_argument("--bb-period", type=int, nargs='+', default=[15, 20, 25], help="ボリンジャーバンド期間")
    parser.add_argument("--bb-std-dev", type=float, nargs='+', default=[1.5, 2.0, 2.5], help="ボリンジャーバンド標準偏差倍数")
    ...
    if args.output and results:
        ...
        output_data = {
            ...
            'summary': {
                ...
                'best_macd_signal': best_result['macd_signal'] if best_result else None,
