...
def grid_search(
        ...
        if short < long_w and long_w / short >= min_ratio:
            ...
            if min_lookback + 100 < len(price_data):
                for stop_loss in stop_loss_percentages:
                    valid_combinations.append((short, long_w, stop_loss, min_lookback))
    ...
    for idx, (short, long_w, stop_loss, lookback) in enumerate(valid_combinations, 1):
        agent_id = f"ma_agent_s{short}_l{long_w}_sl{int(stop_loss*100)}pct"
        ...
        try:
            ...
            if idx % max(1, total_combinations // 20) == 0 or idx == total_combinations:
                ...
                stop_loss_str = f"{best_result['stop_loss_percentage']*100:.0f}%" if best_result['stop_loss_percentage'] is not None else "なし"
            ...
            print(f"エラー: short={short}, long={long_w}, stop_loss={stop_loss}: {e}")
            continue
    ...
    print("トップ20の結果:")
    print("-" * 100)
    print(f"{'Rank':<5} {'Short':<7} {'Long':<7} {'Stop Loss':<10} {'Profit%':<12} {'Profit$':<12} {'Trades':<8}")
    print("-" * 100)
    ...
    for rank, result in enumerate(results[:20], 1):
        ...
        print(f"{rank:<5} "
              f"{result['short_window']:<7} "
              f"{result['long_window']:<7} "
              f"{stop_loss_str:<10} "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="損失確定とウィンドウサイズのグリッドサーチ")
    parser.add_argument(
        "--csv",
        type=str,
    ...
    parser.add_argument(
        ...
        help="テストする損失確定パーセンテージ（デフォルト: 0.03 0.05 0.07 0.10 0.12）"
    )
    parser.add_argument(
        "--short-window",
        type=int,
        nargs='+',
        default=[5, 10, 20, 40, 70, 100],
        help="テストする短期移動平均のウィンドウサイズ（デフォルト: 5 10 20 40 70 100）"
    )
    parser.add_argument(
        "--long-window",
        type=int,
        nargs='+',
        default=[100, 300, 1000, 3000, 5000],
        help="テストする長期移動平均のウィンドウサイズ（デフォルト: 100 300 1000 3000 5000）"
    )
    parser.add_argument(
        "--initial-balance",
        type=float,
